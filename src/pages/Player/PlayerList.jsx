...
const PlayerList = ({ isLoading, isError, searchQuery, showInactivePlayers, selectedTeam, selectedClub, players }) => {
    ...
    if (isError) {
        return <div className="text-center p-4 text-red-600 dark:text-red-400">Error loading players</div>;
    }

    if (filteredPlayers.length === 0) {
        return <div className="text-center p-4 font-semibold text-gray-700 dark:text-gray-300">No players found</div>;
    }

    return (
        <div className="grid gap-4 grid-cols-2 xs-1:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 p-4">
            {filteredPlayers.map((player, index) => (
                <UserPlayerCard key={index} player={player} />
            ))}
        </div>
    );
};

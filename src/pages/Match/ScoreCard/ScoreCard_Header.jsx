...
const ScoreCard_Header = ({ matchInfo }) => {
    ...
    return (
        <div className="mx-auto bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 overflow-hidden mb-6">
            <div className="p-4 py-6 flex flex-col md:flex-row justify-between items-center border-b border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
                <div className="flex md:flex-col lg:flex-row px-4 border-gray-400 min-w-[350px] gap-20 md:gap-0 lg:gap-20 items-center py-6 ">
                    <a href="/teams/33718">
                        {matchInfo?.status === 'live' || matchInfo?.status === "completed" ?
                            <img className="w-20 mb-2" src={matchInfo?.innings?.[0]?.team?.teamLogo} alt="Logo" />
                            :
                            <img className="w-20 mb-2" src={matchInfo?.teams?.[0]?.teamLogo} alt="Logo" />
                        }
                    </a>
                    <div>
                        {matchInfo?.status !== 'live' && matchInfo?.status !== "completed" &&
                            <h1 className="text-3xl font-bold text-gray-800 dark:text-gray-100 mb-1" itemProp="name">{matchInfo?.teams?.[0]?.teamName}</h1>
                        }
                        {matchInfo?.status !== 'scheduled' &&
                            <>
                                <h1 className="text-3xl font-bold text-gray-800 dark:text-gray-100 mb-1" itemProp="name">{matchInfo?.innings?.[0]?.team?.teamName}</h1>
                                <div className="text-sm text-gray-600 dark:text-gray-300 font-semibold">Runs: <span className="text-gray-800 dark:text-gray-100 font-bold text-lg">{matchInfo?.innings?.[0]?.runs}/{matchInfo?.innings?.[0]?.wickets}</span></div>
                                <div className="text-sm text-gray-600 dark:text-gray-300 font-semibold">Overs:
                                    ...
                                    {matchInfo?.innings?.[matchInfo?.currentInning - 1] === 0 &&
                ...
                <span className='border-2 text-base font-semibold w-12 flex justify-center items-center h-12 w-12 rounded-full p-4 dark:text-gray-200'>V/S</span>
                <div className="flex md:flex-col lg:flex-row px-4 border-gray-400 min-w-[350px] gap-20 md:gap-0 lg:gap-20 items-center py-6 ">
                    <a href="/teams/33781" data-team-id="33781">
                        {matchInfo?.status === 'live' || matchInfo?.status === "completed" ?
                            <img className="w-20 mb-2" src={matchInfo?.innings?.[1]?.team?.teamLogo} alt="Logo" />
                            :
                            <img className="w-20 mb-2" src={matchInfo?.teams?.[1]?.teamLogo} alt="Logo" />
                        }
                    </a>
                    <div>
                        {matchInfo?.status !== 'live' && matchInfo?.status !== "completed" &&
                            <h1 className="text-3xl font-bold text-gray-800 dark:text-gray-100 mb-1" itemProp="name">{matchInfo?.teams?.[1]?.teamName}</h1>
                        }
                        {matchInfo?.currentInning === 2 &&
                            <>
                                <h1 className="text-3xl font-bold  text-gray-800 dark:text-gray-100 mb-1" itemProp="name">{matchInfo?.innings?.[1]?.team?.teamName}</h1>
            ...
            <div itemProp="description" className="bg-gray-100 dark:bg-gray-700 p-4 text-center">
                {matchInfo?.status === 'live' && matchInfo?.currentInning ?
                    (
                        <>
                            {matchInfo?.currentInning === 1 ?
                                <h1 className="text-base font-semibold text-gray-600 dark:text-gray-200">
                        ...
                        matchInfo?.result?.winner ? (
                            ...
                            (
                                <div className='flex gap-2 justify-center'>
                                    <h1 className="text-base font-semibold text-gray-600 dark:text-gray-200">
                                        Scheduled on {formatDate(matchInfo?.date)}
                                    </h1>
                                    <h1 className="text-base font-semibold text-gray-600 dark:text-gray-200">
                                        at {matchInfo?.time}
                                    </h1>

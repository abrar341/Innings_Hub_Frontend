...
const RegisterTeamToTournamentDialog = ({ tournamentId }) => {
    ...
    tournamentId = "66d34efebd6b2b67fba0a424"; // For testing purpose
    ...
    const handleSelectTeam = (teamId) => {
        setSelectedTeams((prevSelected) => {
            const updatedSelection = prevSelected.includes(teamId)
                ? prevSelected.filter((id) => id !== teamId)
                : [...prevSelected, teamId];
            console.log("Updated Selected Teams:", updatedSelection);
    ...
    const handleSubmit = async (e) => {
        ...
        // Log the selectedTeams immediately before submission to capture the latest state.
        console.log("Form is being submitted with selected teams:", selectedTeams);

        if (selectedTeams.length === 0) {
            console.log("No teams selected!");
            toast.error("Please select at least one team before proceeding.");
            return;
        }
            ...
            <DialogContent className="max-w-lg w-full bg-gray-800 bg-opacity-50 backdrop-filter backdrop-blur-xl rounded-2xl shadow-xl overflow-hidden">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <button
                        ...
                        // whileTap={{ scale: 0.95 }}
                        type="submit"
                        className="w-full bg-gradient-to-r from-green-500 to-emerald-600 text-white font-bold py-3 px-4 rounded-lg shadow-lg hover:from-green-600 hover:to-emerald-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-50 bg-red-200"
                    >
